<!DOCTYPE html>
<html lang="en">

<head>

<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
<meta name="description" content="">
<meta name="author" content="">

<title>NewProject - The initial base code by CodeInitializer.COM</title>

</head>

<h1>Your Next Big App Starts Here</h1>

<p>This code structure was created by  <a target='_blank' href='http://www.codeinitializer.com'>http://www.codeinitializer.com</a></p>

<ul>
    <li>/data - retrieves the listData values via GET method.
    <li>/data/{string} - adds data to the listData values GET method.
    <li>/data - adds data to the listData values POST method.
    <li>/data - removes all the listData values DELETE method.
</ul>
</html>

